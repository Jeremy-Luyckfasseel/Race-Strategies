...
| `test_ui_leaderboard.js` | 48 assertions. **Renders the real component.** Rename by ✎, double-click, blur and Escape; ★ marking; that a car keeps its colour when the running order changes; the interval column; that a long team name still renders on the busiest row; a 12-car field. |
